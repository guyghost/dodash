// Walk-forward D3-P2 — protocole models/strategy-permission.md §10
// (pré-enregistré a priori ; deltas vs D3-P : porte médiane retirée,
// contrôle d'effet au régime de DÉCISION via passedByRegime INV-P6).
...
// varie). Sélection par train : portes CS4 (dd ≤ 10 %, turnover ≤ 10,
// feeRate ≤ 1 %, run actif — SANS borne médiane) + argmax return
// (défaut C0). Critères W1-W3 sur folds propres. WF3-P : C0 reproduit
// les baselines. Contrôle d'effet : Δdenied > 0 + passedByRegime nul
// sur régimes interdits (C1: BEARISH ; C2: BEARISH+RANGE).
...
interface WindowMeasurement {
  ...
  // INV-P6 §10 — contrôle d'effet au régime de DÉCISION : C1 ⇒ zéro
  // décision passée en BEARISH ; C2 ⇒ zéro en BEARISH et RANGE,
  // quelle que soit la stratégie. Les fills résiduels (latence T+1)
  // restent affichés en info, hors critère.
  readonly deniedRsi: number;
  readonly passedByRegime: Readonly<Record<string, number>>;
  readonly entryFillsByRegime: Readonly<Record<string, number>>;
}

// Portes CS4 (§10 : dd/turnover/fee/run actif — SANS borne médiane,
// indiscriminante pour des candidats à confiance identique, cf. §8)
// + argmax return, défaut C0.
...
for (const year of START_YEARS) {
  ...
  for (const candidateKey of CANDIDATE_KEYS) {
    ...
    const measurement: WindowMeasurement = {
      ...
      passedByRegime: replay.value.regimeGating?.passedByRegime ?? {},
      entryFillsByRegime,
    };
...
// Contrôle d'effet §10 — Δdenied rsi > 0 là où la policy peut mordre,
// et zéro décision PASSÉE en régime interdit (INV-P6, régime de
// décision) : C1 ⇒ passedByRegime[BEARISH] = 0 ; C2 ⇒ = 0 en BEARISH
// et RANGE. Les fills résiduels (latence T+1, §8) sont hors critère.
console.log("\n== Contrôle d'effet (policy mord, régime de décision) ==");
let effectOk = true;
for (const year of START_YEARS) {
  ...
  const c1Ok =
    (!c1CanBite || delta1 > 0) && (c1.passedByRegime["BEARISH"] ?? 0) === 0;
  const c2Ok =
    (!c2CanBite || delta2 > 0) &&
    (c2.passedByRegime["BEARISH"] ?? 0) === 0 &&
    (c2.passedByRegime["RANGE"] ?? 0) === 0;
  effectOk = effectOk && c1Ok && c2Ok;
  console.log(
    `${year}: C1 Δdenied ${delta1} passedBEARISH ${c1.passedByRegime["BEARISH"] ?? 0} ${c1Ok ? "OK" : "FAIL"}${c1CanBite ? "" : " (aucun jour BEARISH)"} | C2 Δdenied ${delta2} passedBEARISH/RANGE ${c2.passedByRegime["BEARISH"] ?? 0}/${c2.passedByRegime["RANGE"] ?? 0} ${c2Ok ? "OK" : "FAIL"}${c2CanBite ? "" : " (aucun jour BEARISH/RANGE)"}`,
  );
}
...
console.log(
  `\nVERDICT D3-P2 : ${verdict ? "VALIDÉ" : "DÉCLASSÉ"} (W1 ${w1 ? "P" : "F"} · W2 ${w2 ? "P" : "F"} · W3 ${w3 ? "P" : "F"} · WF3-P ${wf3Ok ? "P" : "F"} · effet ${effectOk ? "P" : "F"})`,
);
